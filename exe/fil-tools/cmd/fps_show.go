...
func init() {
	fpsShowCmd.Flags().StringP("cid", "c", "", "cid of the data to pin")
	fpsShowCmd.Flags().StringP("token", "t", "", "wallet address used to store the data")

	fpsCmd.AddCommand(fpsShowCmd)
}

var fpsShowCmd = &cobra.Command{
	Use:   "show",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		s.Start()
		info, err := fcClient.FPS.Show(ctx, c)
		s.Stop()
		checkErr(err)
