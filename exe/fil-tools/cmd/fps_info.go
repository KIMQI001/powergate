...
func init() {
	fpsInfoCmd.Flags().StringP("token", "t", "", "token of the request")

	fpsCmd.AddCommand(fpsInfoCmd)
}

var fpsInfoCmd = &cobra.Command{
	Use:   "info",
	Short: "Get info from fps instance",
	Long:  `Get info from fps instance`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		s.Start()
		info, err := fcClient.FPS.Info(ctx)
		checkErr(err)
		s.Stop()
