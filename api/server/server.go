...
import (
	...
	"github.com/textileio/fil-tools/fps"
	"github.com/textileio/fil-tools/fps/minerselector/reptop"
	"github.com/textileio/fil-tools/fps/noopauditer"
...
func NewServer(conf Config) (*Server, error) {
	...
	fpsmanager, err := fps.New(txndstr.Wrap(ds, "fps"), wm, dm, reptop, &noopauditer.Auditer{}, ipfs)
	if err != nil {
		return nil, fmt.Errorf("creating fps instance: %s", err)
	}
	...
	fpsService := fps.NewService(fpsmanager)
	...
	go func() {
		...
		fpsPb.RegisterAPIServer(grpcServer, s.fpsService)
